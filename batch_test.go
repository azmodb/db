...
func TestBatchPutAndInsertRecord(t *testing.T) {
	test := func(rec *Record, err error, value interface{}, werr error, length int) {
		if err != werr {
			t.Fatalf("insert: expected error %v, have %v", werr, err)
		}
		if value == nil && rec != nil {
			t.Fatalf("insert: expected <nil> record")
		}
		if value == nil && rec == nil {
			return
		}
		if len(rec.Blocks) != length {
			t.Fatalf("insert: expected blocks #%d, have #%d", length, len(rec.Blocks))
		}
	...
	rec, _, err := b.Insert("k1", []byte("v1.1"), true)
	test(rec, err, nil, nil, 0)
	rec, _, err = b.Insert("k1", []byte("v1.2"), true)
	test(rec, err, []byte("v1.1"), nil, 1)
	rec, _, err = b.Insert("k1", []byte("v1.3"), false)
	test(rec, err, nil, nil, 0)
	rec, _, err = b.Increment("k1", 1, true)
	test(rec, err, nil, errIncompatibleValue, 0)
	rec, _, err = b.Decrement("k1", 1, true)
	test(rec, err, nil, errIncompatibleValue, 0)
	b.Rollback()

	b = db.Next() // test numeric data type
	rec, _, err = b.Increment("n1", int64(1), true)
	test(rec, err, nil, nil, 0)
	rec, _, err = b.Decrement("n1", int64(1), true)
	test(rec, err, int64(1), nil, 1)
	rec, _, err = b.Increment("n1", int64(1), true)
	test(rec, err, int64(0), nil, 1)
	rec, _, err = b.Decrement("n1", int64(1), false)
	test(rec, err, nil, nil, 0)
	rec, _, err = b.Insert("n1", []byte("1"), true)
	test(rec, err, nil, errIncompatibleValue, 0)
	rec, _, err = b.Put("n1", []byte("1"), true)
	test(rec, err, nil, errIncompatibleValue, 0)
	b.Rollback()
